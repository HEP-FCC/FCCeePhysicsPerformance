    ...
    for cat in df:
      for v in fit_vars:
        for m in df[cat]:
            ...
            h[cat][f"{m}_{v}_syst"], bin_edges[v] = create_hist(df[cat][m].sample(frac=0.5), bins, [v],
                                                           ranges=[[fit_vars[v]["low"], fit_vars[v]["high"]]], normalise=True, with_edges=True)
            h[cat][f"{m}_{v}_syst"] = h[cat][f"{m}_{v}_syst"] * yields[cat][m]
            h[cat][f"{m}_{v}_syst"] = h[cat][f"{m}_{v}_syst"] - h[cat][f"{m}_{v}"]
        ...
        plt.ylim(0.,1.1*ymax)
        plt.tight_layout()
        fig.savefig(f"{loc.PLOTS}/sig_shape_syst_{cat}_{v}_template_compare.pdf")
...
#    sys.exit()
    ...
    def get_templates(mu_Bc, mu_Bu, mu_syst_Bc_bc, mu_bb_bc, mu_cc_bc, mu_syst_Bu_bu, mu_bb_bu, mu_cc_bu, var):
        return ( (mu_Bc    * h['bc'][f"Bc2TauNu_{var}"] + 
                  mu_Bu    * h['bc'][f"Bu2TauNu_{var}"] + 
                  mu_syst_Bc_bc *  h['bc'][f"Bc2TauNu_{var}_syst"] +
                  mu_bb_bc * h['bc'][f"bb_{var}"]       +
                  mu_cc_bc * h['bc'][f"cc_{var}"] ) ,

                 (mu_Bc    * h['bu'][f"Bc2TauNu_{var}"] +
                  mu_Bu    * h['bu'][f"Bu2TauNu_{var}"] +
                  mu_syst_Bu_bu *  h['bu'][f"Bu2TauNu_{var}_syst"] +
    ...
    for i in range(0,n_toys):
        ...
        def loss(x):
            ...
            for v in fit_vars:
                template_bc[v], template_bu[v] = get_templates(mu_Bc, mu_Bu, mu_syst_Bc_bc, mu_bb_bc, mu_cc_bc, mu_syst_Bu_bu, mu_bb_bu, mu_cc_bu, v)  
                nll[v] = binned_nll(template_bc[v], template_bu[v], data['bc'][f"{i}_{v}"], data['bu'][f"{i}_{v}"])
                tot_nll += nll[v]

                tot_nll += (np.abs(mu_syst_Bc_bc) + np.abs(mu_syst_Bu_bu))
        ...
        if (bkg_sf == 0): # for testing signal only stats uncert
          ...
          initial_params[6].floating = False
          initial_params[7].floating = False
        ...
        if(n_toys==1):
          for cat in df:
            for v in fit_vars:
                ...
                plt.ylim(0.,1.2*ymax)
                plt.tight_layout()
                fig.savefig(f"{loc.PLOTS}/sig_shape_syst_{cat}_{v}_template_fit_{bkg_sf}bkg_{bkg_syst}Syst_{nz}Z.pdf")


    #Store toy results to json
    if(n_toys!=1):
        with open(f'{loc.JSON}/sig_shape_syst_toy_simultaneous_template_fit_results_{bkg_sf}bkg_{bkg_syst}Syst_{nz}Z.json', 'w') as fp:
