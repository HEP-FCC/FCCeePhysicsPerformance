...
def buildModel():
  ...
  if (sigFunc == 'CBR'):
    ...
    signal = ROOT.RooCrystalBall("Double Crystal Ball", "Double Crystal Ball PDF", x, mean, sigma_L, sigma_R, alpha_L, n_L,alpha_R, n_R)
    ...
    signal = ROOT.RooBernstein("Bern4", "Bern4", x, ROOT.RooArgList(sig_p0, sig_p1, sig_p2, sig_p3, sig_p4))
  elif (sigFunc == "Pow"):
  ...
  elif (sigFunc == "DCB_Pol2"):
    DSCB = ROOT.RooCrystalBall("DSCB", "Double Sided Crystal Ball PDF", x, mean, sigma, alpha_L, n_L,alpha_R, n_R)
    POL2 = ROOT.RooPolynomial("POL2", "Pol2", x, ROOT.RooArgList(sig_p0, sig_p1, sig_p2))
    signal = ROOT.RooAddPdf("DSCB_POL2","DSCB + POL2",RooArgList(DSCB,POL2),sigfrac)
  elif (sigFunc == "DCBA_Pol2"):
    DSCB = ROOT.RooCrystalBall("DSCB", "Double Sided Crystal Ball PDF", x, mean, sigma_L, sigma_R, alpha_L, n_L,alpha_R, n_R)
    POL2 = ROOT.RooPolynomial("POL2", "Pol2", x, ROOT.RooArgList(sig_p0, sig_p1, sig_p2))
    signal = ROOT.RooAddPdf("DSCBA_POL2","DSCB + POL2",RooArgList(DSCB,POL2),sigfrac)
  elif (sigFunc == "DCBG"):
    DSCB = ROOT.RooCrystalBall("DSCB", "Double Sided Crystal Ball PDF", x, mean, sigma, alpha_L, n_L,alpha_R, n_R)
    G = ROOT.RooGaussian("Gaussian", "Gaussian", x, mean2, sigma2)
    signal = ROOT.RooAddPdf("DSCBG","DSCB + Gaussian",RooArgList(DSCB,G),sigfrac)
  elif (sigFunc == "DCBAG"):
    DSCB = ROOT.RooCrystalBall("DSCB", "Double Sided Crystal Ball PDF", x, mean, sigma_L, sigma_R, alpha_L, n_L,alpha_R, n_R)
    G = ROOT.RooGaussian("Gaussian", "Gaussian", x, mean2, sigma2)
    signal = ROOT.RooAddPdf("DSCBAG","DSCBA + Gaussian",RooArgList(DSCB,G),sigfrac)
  elif (sigFunc == "2CBG"):
    CBR = ROOT.RooCBShape("CBR", "Crystal Ball right tail", x, mean, sigma, alphaR, n_R)
    CBL = ROOT.RooCBShape("CBL", "Crystal Ball left tail", x, mean, sigma, alphaL, n_L)
    G = ROOT.RooGaussian("Gaussian", "Gaussian", x, mean2, sigma2)
    signal = ROOT.RooAddPdf("2CBG","CBR + CBL + Gaussian",RooArgList(CBR,CBL,G),RooArgList(sigfrac1,sigfrac2))
  else:
...
def plot():
  ...
  #xframe.getAttText().SetTextSize(0.1)
  #if not SBMode:
...
if __name__ == "__main__":
  ...
  mean = ROOT.RooRealVar("mean", "mean", peak, peak - 0.3, peak + 0.3)
  #mean = ROOT.RooRealVar("mean", "mean", 125.0, 124.7, 125.3)
  sigma = ROOT.RooRealVar("sigma", "width", 0.25, 0.2, 0.4)
  #sigma = ROOT.RooRealVar("sigma", "width", 0.25, 0.2, 3.0)
  alphaR = ROOT.RooRealVar("alphaR", "alpha of Crystal Ball", -1.0, -5.0, -0.01)
  alphaL = ROOT.RooRealVar("alphaL", "alpha of Crystal Ball", 1.0, 0.01, 5.0)
  n = ROOT.RooRealVar("n", "n of Crystal Ball", 5.0, 0.0, 200.0)
  mean2 = ROOT.RooRealVar("mean2", "mean of gaussian", peak, peak - 0.4, peak + 0.4)
  sigma2 = ROOT.RooRealVar("sigma2", "width of gaussian", 0.8, 0.1, 1.0)
  ...
  #Z region
  sigma_L = ROOT.RooRealVar("sigma_L", "width", 0.25, 0.1, 2.0)
  sigma_R = ROOT.RooRealVar("sigma_R", "width", 0.25, 0.1, 0.4)
  #sigma_L = ROOT.RooRealVar("sigma_L", "width", 0.25, 0.0, 2.0)
  #sigma_R = ROOT.RooRealVar("sigma_R", "width", 0.25, 0.1, 1.0)
  alpha_L = ROOT.RooRealVar("alpha_L", "alpha of Crystal Ball", 0.7, 0.1, 5.0)
  n_L = ROOT.RooRealVar("n_L", "n of Crystal Ball", 5.0, 0.0, 20.0) 
  alpha_R = ROOT.RooRealVar("alpha_R", "alpha of Crystal Ball", 0.7, 0.0, 5.0)
  n_R = ROOT.RooRealVar("n_R", "n of Crystal Ball", 5.0, 0.0, 10.0) 
  
  #alphaL = ROOT.RooRealVar("alphaL", "alpha of Crystal Ball", 1.93, 1.93, 1.93)
  #alphaR = ROOT.RooRealVar("alphaR", "alpha of Crystal Ball", -0.1324, -0.1324, -0.1324)
  #n_L = ROOT.RooRealVar("n_L", "n of Crystal Ball", 1.64, 1.64, 1.64) 
  #n_R = ROOT.RooRealVar("n_R", "n of Crystal Ball", 2.11, 2.11, 2.11)
  ...
  sig_p0 = ROOT.RooRealVar("sig_p0", "p0 of polynomial", 1, -100.0, 100.0)
  sig_p1 = ROOT.RooRealVar("sig_p1", "p1 of polynomial", 1, -100.0, 100.0)
  sig_p2 = ROOT.RooRealVar("sig_p2", "p2 of polynomial", 1, -100.0, 100.0)
  sig_p3 = ROOT.RooRealVar("sig_p3", "p3 of polynomial", 1, -100.0, 100.0)
  sig_p4 = ROOT.RooRealVar("sig_p4", "p4 of polynomial", 1, -100.0, 100.0)

  p0 = ROOT.RooRealVar("p0", "p0 of polynomial", -5.0, 5.0)
  p1 = ROOT.RooRealVar("p1", "p1 of polynomial", -2.0, 2.0)
  p2 = ROOT.RooRealVar("p2", "p2 of polynomial", -10.0, 10.0)
  p3 = ROOT.RooRealVar("p3", "p3 of polynomial", -3.0, 3.0)
  p4 = ROOT.RooRealVar("p4", "p4 of polynomial", -3.0, 3.0)
 
  p0 = ROOT.RooRealVar("p0", "p0 of polynomial", 0.99, 0.99)
  p1 = ROOT.RooRealVar("p1", "p1 of polynomial", 0.0046, 0.0046)
  p2 = ROOT.RooRealVar("p2", "p2 of polynomial", -0.0000657, -0.0000657)

  sigfrac = ROOT.RooRealVar("sigfrac","fraction of component 1 in signal",0.5,0.,1.)
  sigfrac1 = ROOT.RooRealVar("sigfrac1","fraction of component 1 in signal",0.3,0.,1.0)
  sigfrac2 = ROOT.RooRealVar("sigfrac2","fraction of component 1 in signal",0.3,0.,1.0)
  sigfrac1 = ROOT.RooRealVar("sigfrac1","fraction of component 1 in signal",0.674,0.674,0.674)
  sigfrac2 = ROOT.RooRealVar("sigfrac2","fraction of component 1 in signal",0.174,0.174,0.174)
  ...
  if hasattr(param, 'n_L'):
    n_L = param.n_L
  if hasattr(param, 'alpha_R'):
    alpha_R = param.alpha_R
  if hasattr(param, 'n_R'):
    n_R = param.n_R
  if hasattr(param, 'p0'):
    p0 = param.p0
  if hasattr(param, 'p1'):
  ...
  if hasattr(param, 'sig_p4'):
    sig_p4 = param.sig_p4
  if hasattr(param, 'sigfrac'):
    sigfrac = param.sigfrac
